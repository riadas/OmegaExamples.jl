...
function optim_all_vars_no_exo(bin_size::Int64, batch_size::Int=4, maxiters::Int=150, lr::Float64=0.01)
  u0, ode_data = prepare_all_data_meals_hypo(bin_size)
  samples, losses, best_pred = model(ode_data, batch_size, max_iters, lr)
...
# glucose, steps, bolus, meals
function optim_all_vars_exo(bin_size::Int64, batch_size::Int=4, maxiters::Int=150, lr::Float64=0.01)
  u0, ode_data = prepare_all_data_meals_hypo(bin_size)
  non_exo_data = ode_data[1:2, :]
  exo_data = ode_data[3:4, :]
  samples, losses, best_pred = model_exo(non_exo_data, exo_data, batch_size, max_iters, lr)
end
...
function bayes_two_vars_no_exo(bin_size::Int64)
  u0, ode_data = prepare_all_data(bin_size)
  samples, losses = model_bayes(ode_data, batch_size, max_iters, lr)
  samples, losses
end
...
function bayes_all_vars_exo(bin_size::Int64)
  ...
  non_exo_data = ode_data[1:2, :]
  exo_data = ode_data[3:4, :]
  samples, losses = model_bayes_exo(non_exo_data, exo_data, batch_size, max_iters, lr)
end
...
function bayes_one_var_exo(var::String)
  u0, ode_data = prepare_data(var, bin_size)
  if var == "basis_steps"
    samples, losses = model_bayes_exo(ode_data, batch_size, max_iters, lr)
  else
    non_exo_data = ode_data[1, :]
    exo_data = ode_data[2, :]
    samples, losses = model_bayes_exo(non_exo_data, exo_data, batch_size, max_iters, lr)
